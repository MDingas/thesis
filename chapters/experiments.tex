...
\begin{tabular}{|l|L|}
    ...
    ALTO                       & \multicolumn{1}{m{11cm}|}{Retrieve a network map from its ALTO server and discover the attributed PID to both request and candidate clients, choosing randomly between the candidate peers that share the same PID as the candidate} \\ \hline
\end{tabular}
...
\begin{tabular}{|l|l|L|}
    ...
    Network Traffic          & Megabytes          & \multicolumn{1}{m{10cm}|}{Total amount of traffic that passed through the network} \\ \hline
\end{tabular}
...
\begin{tabular}{|l|L|}
    ...
    Random                      & \multicolumn{1}{m{11cm}|}{Randomly select among all available servers}              \\ \hline
    Probe                       & \multicolumn{1}{m{11cm}|}{Probe for available TCP throughput and existing delay between all available servers, and select the one with most throughput with delay below 20 ms}                                                                                                     \\ \hline
    ALTO                        & \multicolumn{1}{m{11cm}|}{Retrieve a network map, endpoint property map, and cost map from the ALTO server in the HTTP servers AS, and choose, if possible, a server that is not overloaded - i.e., whose memory and cpu loads are not past 90\% - whose delay is below 20ms, and which has an historical throughtput of 30 Mbps or more. If multiple servers match these requirements, choose the one that has the least routing cost among them}                                \\ \hline
\end{tabular}
\caption{x}
\end{table}
...
\begin{tabular}{|l|l|L|}
    \hline
    \textbf{Measurement}   & \textbf{Units}     & \multicolumn{1}{m{8cm}|}{\textbf{Description}}                                         \\ \hline
    Transfer Time          & Seconds            & \multicolumn{1}{m{8cm}|}{Total amount of time required to transfer the requested file} \\ \hline
    CPU Server Load        & Percentage         & \multicolumn{1}{m{8cm}|}{Average CPU load of all mirror servers}                       \\ \hline
    Memory Server Load     & Percentage         & \multicolumn{1}{m{8cm}|}{Average Memory load of all mirror servers}                    \\ \hline
    Network Traffic        & Megabytes          & \multicolumn{1}{m{8cm}|}{Total amount of traffic that passed through the network}      \\ \hline
\end{tabular}
\caption{x}
\end{table}
...
\begin{tabular}{|l|L|}
    ...
    Random                    & \multicolumn{1}{m{11cm}|}{Randomly select among all available servers}   \\ \hline
    Probe                     & \multicolumn{1}{m{11cm}|}{Probe for available TCP throughput and existing delay between all available servers, and select the one with most throughput with delay below 20 ms}                                                                                          \\ \hline
    ALTO                      & \multicolumn{1}{m{11cm}|}{Retrieve a network map and cost map from the ALTO server in the HTTP servers AS, and choose the one that maximizes throughput in the calendar entry where the transfer will occur}                                                            \\ \hline
\end{tabular}
\caption{x}
