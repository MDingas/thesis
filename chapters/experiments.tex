...
\end{table}
    ...
    The variable actions to be tested are how the client selects which candidate peer, from the available pool, will be selected to serve each given file fragment.
...
\begin{tabular}{|l|L|}
    ...
    Random                     & \multicolumn{1}{m{11cm}|}{Randomly select among all available peers}                                                     \\ \hline
    RTT                        & \multicolumn{1}{m{11cm}|}{Select the peer with the smallest average \gls{rtt} measurement in 10 ping commands}     \\ \hline
    ALTO - Local               & \multicolumn{1}{m{11cm}|}{Retrieve the local network map from the local \gls{alto} server and select the peer whose \gls{pid} matches the requesting peer, choosing randomly if multiple matches exist within that group}                                                                                                           \\ \hline
    ALTO - Global              & \multicolumn{1}{m{11cm}|}{Retrieve the global multi-cost endpoint cost map from the local \gls{alto} server by querying for the throughput and one way delay metrics, and selecting the peer that maximizes throughput with a delay no bigger than 5 milliseconds.}                                                  \\ \hline
\end{tabular}
...
\begin{tabular}{|l|l|L|}
    ...
    Transfer time            & Seconds            & \multicolumn{1}{m{10cm}|}{Total amount of time required to retrieve all the file fragments} \\ \hline
...
\begin{tikzpicture}
    ...
    \begin{axis}[
        ...
        bar width=8,
        xlabel={Traffic influx by region (megabytes)},
        symbolic y coords={{ALTO-Global},{ALTO-Local},{RTT},{Random}},
...
\end{figure}
    ...
    The approach using global \gls{alto} resources had concrete information regarding expected path throughput and one way delay, but no strategy to localize traffic, instead favoring connections that maximize throughput with delay that did not compromise its \gls{qos} standards.
...
\begin{tabular}{|l|L|}
    ...
    Bandwidth                 & \multicolumn{1}{m{11cm}|}{Request the file using wget and, after five seconds, which is sufficient to get a view of the transfer performance, restart the transfer unless a value of 10 Mbps or higher was reported}                                                                                                                                                                          \\ \hline
    ALTO                      & \multicolumn{1}{m{11cm}|}{Retrieve a calendar cost map from the local \gls{alto} server of expected path delay from itself to the server, and initiate immediately when the available bandwidth is expected to be 10 Mbps or higher}                                                                                    \\ \hline
\end{tabular}
...
\begin{tikzpicture}
    \begin{axis}[ 
        xbar,xmin=0,xmax=1500,
        xlabel={Transfer times (s)},
        symbolic y coords={{ALTO-Global},{Bandwidth},{Immediate}},
        xtick={0, 500, 1000, 1500},
        ytick=data,
        nodes near coords, nodes near coords align={horizontal},
        ytick=data,
    ]
    \addplot[fill=lightblue] coordinates {(641.998,{Bandwidth}) (457.186,{Immediate}) (636.456,{ALTO})};
    \end{axis}
\end{tikzpicture} %width=6cm,height=7.59cm
...
\begin{tabular}{|l|L|}
    ...
    Bandwidth                 & \multicolumn{1}{m{11cm}|}{Probe the mirrors for maximum path bandwidth using iperf3 and average \gls{rtt} using 10 ping commands, and select the one with most bandwidth whose delay doesn't cross 100 ms, choosing randomly if multiple servers fulfill the criteria.}   \\ \hline
    ALTO                      & \multicolumn{1}{m{11cm}|}{Retrieve the global endpoint property maps and endpoint cost maps by querying for the CPU load property, and both throughput and one way delay cost metrics, respectively, and choose the one which maximizes throughput, assuming that it has delay below 100 milliseconds and processing load is below 50, so not to overload the server \%.} \\ \hline
\end{tabular}
\caption{Client algorithms to be tested in scenario 3}
\label{table:s3-client-methods}
\end{table}

    Table \ref{table:s3-measurements} presents the measurements that will be collected during the experiment runs.
...
\end{figure}
    ...
    It appears that the approach that probed for path bandwidth had considerable impact on the generated traffic influx, and the remaining approaches had a negligible amount of overhead traffic in comparison.
...
\end{figure}
    ...
    The \gls{alto} approach was able to get immediate \gls{isp} input in the form of a combined multi-\gls{alto} domain effort.
